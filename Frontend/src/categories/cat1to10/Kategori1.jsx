...
import { faCircleInfo, faXmarkCircle } from "@fortawesome/free-solid-svg-icons"; // import an icon
...
import HealthClaimInputs from "../../components/HealthClaimInputs";
...
const Kategori1 = ({ 
  ...
  const [highFibre, setHighFibre] = useState(null);
  const [sourceOfProtein, setSourceOfProtein] = useState(null);
  const [reducedFat, setReducedFat] = useState(null);
  const [reducedSaturatedFat, setReducedSaturatedFat] = useState(null);
  const [reducedSalt, setReducedSalt] = useState(null);
  ...
  */
  ...
  // Creates a new product object and sends it to the backend when the form is submitted
    ...
    */
    ...
    const claims = [
      ...
      { name: 'Høyt Fiberinnhold', value: highFibre },
      { name: 'Inneholder kilder av Protein', value: sourceOfProtein },
      { name: 'Redusert Fettinnhold', value: reducedFat },
      { name: 'Redusert mettet Fettinnhold', value: reducedSaturatedFat },
      { name: 'Redusert Saltinnhold', value: reducedSalt },
    ];
    ...
    useEffect(() => {
      checkClaims(claims);
    }, [lowEnergy, lowFat, fatFree, lowSaturatedFat, saturatedFatFree, lowSugars, sugarsFree, withNoAddedSugars, highFibre, sourceOfProtein, reducedFat, reducedSaturatedFat, reducedSalt]);
  ...
  const onClick = (e) => {
    ...
    */
    ...
    setReducedFat(Check.claimReducedFat(nutrition.fett))
    setReducedSaturatedFat(Check.claimReducedSaturatedFat(nutrition.mettede))
    setReducedSalt(Check.claimReducedSalt(nutrition.salt))
    ...
    if (Check.claimReducedSalt(nutrition.salt)) {
      hasLowSalt(true);
    } else {
      hasLowSalt(false);
    }
                ...
                <th scope="row" className="table-font">
                  {fett ? (
                    <Tooltip
                      ...
                      arrow
                    >
                      <div className="icon" style={{ paddingRight:'0.5rem'}}>
                      <img
                        src={errorKeyhole}
                        alt="Error keyhole"
                        style={{width:'1.5rem', height: 'auto'}}></img>
                      </div>
                    </Tooltip>
                  ) : null}
                  {fettNull ? (
  ...
  {buttonClicked && showResults && showNokkelhulletResults ? (
    ...
    <ResultNokkelhulletFulfilled category={"Kategori1"} className="container nøkkelhullet-food-result-container">
  ...
  {buttonClicked && showResults && (sourceOfProtein || highFibre || lowEnergy || lowFat || fatFree || lowSaturatedFat || saturatedFatFree || lowSugars || sugarsFree || withNoAddedSugars || reducedFat || reducedSaturatedFat || reducedSalt) ? (
    <div className={`container ernæringspåstander-food-result-container-${showErnaeringsResults ? 'true' : 'false'}`}
      ...
      claimsToShow={{
        ...
        reducedSaturatedFat={reducedSaturatedFat}
