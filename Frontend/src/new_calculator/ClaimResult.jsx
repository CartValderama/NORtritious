...
export const CLAIMS_CONFIG = {
  ...
  highFibre: {
    ...
    metText: 'Dette produktet inneholder høyst 6 g fiber per 100 g eller høyst 3 g fiber per 100 kcal.',
    notMetLines: [
      'Produktet må inneholde 6 g fiber per 100 g eller høyst 3 g fiber per 100 kcal. *Husk å benytte kcal som enhet for energi.',
    ],
  },
  sourceOfFibre: {
    name: 'Kostfiberkilde',
...
const ClaimResult = ({ name, met, metText, notMetLines }) => {
  const textLines = Array.isArray(metText) ? metText : [metText];

  const textColor = met ? '#0f5132' : '#842029';

  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', color: textColor }}>
      <p>
        <b>
          ** Produktet oppfyller {met ? '' : 'ikke '}kravet for{' '}
          <span style={{ textDecoration: 'underline' }}>"{name}"</span> påstanden.
        </b>
      </p>
      <div style={{ width: '100%' }}>
        {met ? (
          textLines.map((line, i) => (
            <p key={i} style={i === 0 ? { paddingLeft: '20px' } : undefined}>
              {i === 0 && (
                <FontAwesomeIcon
                  icon={faCheck}
                  style={{ color: textColor, marginRight: '5px', fontWeight: 'bold', fontSize: '1.2em' }}
                />
              )}
              {line}
            </p>
