      ...
      {isExpanded && (
        <div>
          ...
          {Object.entries(requirements).map(([key, value]) => value ? (
              <li key={key} style={{ padding: '0.4em', fontWeight: 'bold' }}>{key}: {value}</li>
            ) : null)}
          </ul>
        </div>
      )}
...
function ResultNokkelhulletNotFulfilled({ category, fett, mettede, hvoravSukkerarter, salt }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", backgroundColor: '#f3b7b7', padding: '2.2em', borderRadius: '0.5em' }}>
